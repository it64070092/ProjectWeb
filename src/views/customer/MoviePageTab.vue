                    ...
                    <div class="px-6 py-4 bg-white">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div v-for="cinema in cinemas" :key="cinema.id">
                                <button
                                    class="focus:outline-none border border-gray-300 rounded-lg py-2 px-20 w-full justify-center items-center flex transition duration-150 ease-in-out"
                                    :class="{ 'bg-blue-700 text-white': selectedCinema.id === cinema.id }"
                        ...
                        <div class="flex justify-between" v-if="selectedCinema != ''">
                            ...
                            <select class='border border-gray-700 rounded-lg my-3 py-2 px-10' id="location"
                            ...
                            <button v-if="selectedLocation != null"
                                class="my-2 w-20 border border-gray-700 rounded-lg py-2 px-5"
                    ...
                    <div class="px-6 py-4 bg-white">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div v-for="screen in screens" :key="screen.id">
                                <button
                                    class="focus:outline-none  border border-gray-300 rounded-lg py-2 px-20 w-full justify-center items-center flex transition duration-150 ease-in-out"
                                    :class="{ 'bg-blue-500 text-white': selectedScreen === screen.name }"
                        ...
                        <div class="flex justify-end">
                            <button class="my-2 mx-3 w-20  border border-gray-700 rounded-lg py-2 px-5"
            ...
            <div v-else-if="showDateSelection && selectedScreen != ''">
                <div class="overflow-x-auto py-3 -mx-2">
                    <div class="flex px-2">
                        <div class="flex space-x-2">
                            <button
                                class="bg-white focus:outline-none border border-gray-300 rounded-xl w-full py-2 px-5 justify-center items-center transition duration-150 ease-in-out"
                ...
                <div class='flex px-2' v-if="selectdate != ''">
                    <button v-for="datetime in time" :key='datetime.id'
                        ...
                        :class="{ 'bg-blue-500 text-white': selecttime === datetime.period }"
                        @click="selectTime(datetime.period)">{{ datetime.period }}</button>
                </div>
                <div v-if="selecttime != ''" class='mt-5'>
                    <seatRow ref="seatRow" @seats-selected='onSeatsSelected'
                        :selectedSeats="Seats"
                ...
                <div class="flex justify-end mt-4 text-center">
                    ...
                    <button class="my-2 text-center bg-white border border-gray-700 rounded-lg py-2 px-5"
                        @click="booking();">
                        Confirm
                    </button>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            bookingseats: JSON.parse(bookings) || [],
    ...
    methods: {
        ...
        booking() {
            let newBooking = { user: this.User, movie: this.movie, cinema: this.selectedCinema, location: this.selectedLocation, screen: this.selectedScreen, date: this.selectdate, time: this.selecttime, seats: this.Seats, price: (this.movie.price * this.Seats.length) }
            this.Seats.forEach(element => {
                element.status = 'occupied';
                element.isSelected = false;
            });
            this.bookingseats.push(newBooking);
            localStorage.setItem('booking', JSON.stringify(this.bookingseats))
