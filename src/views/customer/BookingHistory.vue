<template>
    ...
    <div class="bg-gray-700  min-h-screen pt-20">
        <h1 class="text-2xl font-bold text-white pt-5 p-4 bg-gradient-to-r from-blue-800 to-transparent mb-4">Booking
            History</h1>
...
export default {
    ...
    methods: {
        loadBookings() {
            ...
            for (const item in bookings) {
                for (const username in bookings[item].user) {
                    if (bookings[item].user[username].username == user[0].username) {
                        userBookings.push(bookings[item]);
                    }
                }
            }
