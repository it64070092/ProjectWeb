<template>
    ...
    <router-link :to="{ name: 'movie', params: { id: movie.id } }"  class="w-full md:w-1/2">
        <div class="relative overflow-hidden rounded-lg shadow-md">
            <div class="absolute top-0 left-0 p-2">
                <button @click="editMovie(movie.id)" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded">
                    Edit
                </button>
            </div>
            <div class="absolute top-0 left-14 p-2">
                <button @click="deleteMovie(movie.id)" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded">
                    Delete
                </button>
            </div>
            <img :src="movie.poster" class="w-full h-auto object-cover object-center" alt="">
            <div class="absolute top-0 right-0 p-4 bg-gradient-to-l from-blue-800 to-transparent rounded-md">
