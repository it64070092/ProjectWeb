                            ...
                            <form @submit.prevent="submitForm">
                                <div class="relative w-full mb-3">
                                    <label class="block uppercase text-gray-700 text-xs font-bold mb-2"
                                        for="grid-password">Username</label><input type="text" v-model="username"
                                ...
                                <div>
                                    <label class="inline-flex items-center cursor-pointer"><input id="customCheckLogin"
                                            type="checkbox" v-model="remember" @click="rememberMe"
...
export default {
    ...
    methods: {
        submitForm() {
            if (this.username == this.UserID[0].username && this.password == this.UserID[0].password) {
                this.succes = true
        ...
        rememberMe() {
            if (!this.remember) {
                this.username = this.UserID[0].username
                this.password = this.UserID[0].password
            }
            else {
                this.username = ''
                this.password = ''
